<div align="center">

<img src="logo.png" alt="Datvex Prompt LAB" width="200"/>

# Datvex Prompt LAB

### O Repositório Profissional e Estruturado de Prompts de IA de Alta Qualidade

&nbsp;

[![Website](https://img.shields.io/badge/Website-Live-black?style=for-the-badge&logo=vercel&logoColor=white)](https://datvex-prompt-lab.vercel.app/)
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repository-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Datvex/Datvex-prompt-LAB)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue?style=for-the-badge&logo=opensourceinitiative&logoColor=white)](https://github.com/Datvex/Datvex-prompt-LAB/blob/main/LICENSE)

<br/>

[![Vercel](https://img.shields.io/badge/Vercel-Deploy-000?style=flat-square&logo=vercel&logoColor=white)](https://vercel.com)
[![TailwindCSS](https://img.shields.io/badge/TailwindCSS-38B2AC?style=flat-square&logo=tailwindcss&logoColor=white)](https://tailwindcss.com)
[![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)

<br/>

[![GitHub Stars](https://img.shields.io/github/stars/Datvex/Datvex-prompt-LAB?style=flat-square&logo=github&logoColor=white&label=Stars&labelColor=181717&color=6e5494)](https://github.com/Datvex/Datvex-prompt-LAB/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/Datvex/Datvex-prompt-LAB?style=flat-square&logo=github&logoColor=white&label=Forks&labelColor=181717&color=2EA043)](https://github.com/Datvex/Datvex-prompt-LAB/forks)
[![GitHub Issues](https://img.shields.io/github/issues/Datvex/Datvex-prompt-LAB?style=flat-square&logo=github&logoColor=white&label=Issues&labelColor=181717&color=CB1B45)](https://github.com/Datvex/Datvex-prompt-LAB/issues)

</div>

---

> 🌐 **Disponível em:** 🇺🇸 EN [English](README.md) | 🇧🇷 BR [Português (Brasil)](README_BR.md) | 🇪🇸 ES [Español](README_ES.md) | 🇫🇷 FR [Français](README_FR.md) | 🇮🇹 IT [Italiano](README_IT.md) | 🇷🇺 RU [Русский](README_RU.md) | 🇨🇳 ZH [中文 (简体)](README_ZH.md) | 🇩🇪 DE [Deutsch](README_DE.md) | 🇮🇳 HI [हिन्दी](README_HI.md) | 🇹🇭 TH [ไทย](README_TH.md) | 🇺🇦 UA [Українська](README_UA.md) | 🇸🇦 AR [العربية](README_AR.md) | 🇯🇵 JA [日本語](README_JA.md) | 🇻🇳 VN [Tiếng Việt](README_VN.md) | 🇧🇬 BG [Български](README_BG.md) | 🇩🇰 DK [Dansk](README_DK.md) | 🇫🇮 FI [Suomi](README_FI.md) | 🇮🇱 IL [עברית](README_IL.md) | 🇭🇺 HU [Magyar](README_HU.md) | 🇮🇩 ID [Bahasa Indonesia](README_ID.md) | 🇰🇷 KR [한국어](README_KR.md) | 🇲🇾 MY [Bahasa Melayu](README_MY.md) | 🇳🇱 NL [Nederlands](README_NL.md) | 🇳🇴 NO [Norsk](README_NO.md) | 🇵🇹 PT [Português (Portugal)](README_PT.md) | 🇷🇴 RO [Română](README_RO.md) | 🇵🇱 PL [Polski](README_PL.md) | 🇸🇰 SK [Slovenčina](README_SK.md) | 🇸🇪 SE [Svenska](README_SE.md) | 🇵🇭 PH [Filipino](README_PH.md) | 🇨🇿 CZ [Čeština](README_CZ.md)

---

## Índice

- [Visão Geral](#visão-geral)
- [Funcionalidades](#funcionalidades)
- [Stack Tecnológica](#stack-tecnológica)
- [Internacionalização](#internacionalização)
- [Contribuições](#contribuições)
- [Licença](#licença)

---

## Visão Geral

O **Datvex Prompt LAB** é um repositório profissional e estruturado que contém prompts de alta qualidade concebidos para tarefas especializadas em múltiplos sistemas de IA. A plataforma disponibiliza uma interface intuitiva com tema escuro para navegar, pesquisar e filtrar prompts por tipo, categoria e etiquetas.

Construído com foco no desempenho, o Datvex proporciona uma experiência ultrarrápida com pesquisa instantânea, deslocamento infinito e um poderoso sistema de filtragem — tudo sem frameworks pesados.

<div align="center">

| ![Text](https://img.shields.io/badge/Texto-60A5FA?style=flat-square&logo=markdown&logoColor=white) | ![Image](https://img.shields.io/badge/Imagem-4ADE80?style=flat-square&logo=image&logoColor=white) | ![Video](https://img.shields.io/badge/Vídeo-FBBF24?style=flat-square&logo=video&logoColor=white) | ![Audio](https://img.shields.io/badge/Áudio-C084FC?style=flat-square&logo=audio&logoColor=white) |
|:---:|:---:|:---:|:---:|
| Geração de Texto | Geração de Imagem | Geração de Vídeo | Geração de Áudio |
| ChatGPT, Grok, Claude, Gemini | Midjourney, DALL-E, Nano Banana | Sora, Kling, Runway | Suno, ElevenLabs |

</div>

---

## Funcionalidades

<table>
  <tr>
    <td width="33%" align="center" valign="top">
      <img src="https://img.shields.io/badge/Biblioteca_Estruturada-60A5FA?style=for-the-badge&logo=stack-overflow&logoColor=white" alt="Biblioteca Estruturada"/><br/><br/>
      Prompts cuidadosamente categorizados para geração de texto, imagem, vídeo e áudio — organizados por tipo, categoria e etiquetas.
    </td>
    <td width="33%" align="center" valign="top">
      <img src="https://img.shields.io/badge/Pesquisa_Instantânea-4ADE80?style=for-the-badge&logo=algolia&logoColor=white" alt="Pesquisa Instantânea"/><br/><br/>
      Pesquisa de texto completo em tempo real com resultados instantâneos à medida que escreve. Encontre o prompt exato em milissegundos.
    </td>
    <td width="33%" align="center" valign="top">
      <img src="https://img.shields.io/badge/Multi_Modelo-FBBF24?style=for-the-badge&logo=openai&logoColor=black" alt="Multi Modelo"/><br/><br/>
      Testado e otimizado para ChatGPT, Claude, Gemini, Midjourney, DALL-E, Cursor, Windsurf e mais.
    </td>
  </tr>
  <tr>
    <td width="33%" align="center" valign="top">
      <img src="https://img.shields.io/badge/Envio_com_Um_Clique-C084FC?style=for-the-badge&logo=rocket&logoColor=white" alt="Um Clique"/><br/><br/>
      Envie qualquer prompt diretamente para o seu modelo de IA preferido com um único clique — sem necessidade de copiar e colar.
    </td>
    <td width="33%" align="center" valign="top">
      <img src="https://img.shields.io/badge/Favoritos-FF6B6B?style=for-the-badge&logo=heart&logoColor=white" alt="Favoritos"/><br/><br/>
      Guarde os seus prompts favoritos para acesso rápido. Tudo guardado localmente com armazenamento persistente.
    </td>
    <td width="33%" align="center" valign="top">
      <img src="https://img.shields.io/badge/11_Idiomas-FF6F61?style=for-the-badge&logo=googletranslate&logoColor=white" alt="Idiomas"/><br/><br/>
      Suporte completo da interface para 11 idiomas: EN, RU, ZH, ES, HI, FR, DE, IT, PT, JA, KO.
    </td>
  </tr>
</table>

---

## Stack Tecnológica

<div align="center">

### Frontend

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwindcss&logoColor=white)

### Infraestrutura

![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)

</div>

### Bibliotecas de UI / Ícones

| Biblioteca | Finalidade |
|:---|:---|
| [TailwindCSS](https://tailwindcss.com) | Framework CSS utilitário |
| [Phosphor Icons](https://phosphoricons.com) | Família de ícones flexíveis para interfaces |
| [Inter](https://rsms.me/inter/) | Tipo de letra limpo e profissional por Rasmus Andersson |

---

## Internacionalização

O Datvex Prompt LAB suporta **11 idiomas** de origem:

| Idioma | Código | Estado |
|:---|:---:|:---:|
| Inglês | `en` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Russo | `ru` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Chinês (Simplificado) | `zh` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Espanhol | `es` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Hindi | `hi` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Francês | `fr` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Alemão | `de` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Italiano | `it` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Português | `pt` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Japonês | `ja` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |
| Coreano | `ko` | ![](https://img.shields.io/badge/concluído-brightgreen?style=flat-square) |

O idioma é armazenado no `localStorage` e persiste entre sessões.

---

## Contribuições

As contribuições são o que torna a comunidade de código aberto incrível. Qualquer contribuição que faça será **muito apreciada**.

### Como Contribuir

1. **Faça Fork** do repositório
2. **Crie** o seu ramo de funcionalidade
   ```bash
   git checkout -b feature/funcionalidade-incrivel
   ```
3. **Faça Commit** das suas alterações
   ```bash
   git commit -m "Adiciona funcionalidade incrível"
   ```
4. **Faça Push** para o ramo
   ```bash
   git push origin feature/funcionalidade-incrivel
   ```
5. **Abra** um Pull Request

### Estilo de Código

- No código, evite excesso de comentários — apenas comentários essenciais ou em áreas particularmente complexas são permitidos
- Utilize classes utilitárias do TailwindCSS para estilização
- Mantenha a consistência do tema escuro
- Mantenha o JavaScript vanilla (sem frameworks)

---

## Licença

Distribuído sob a Licença MIT. Consulte `LICENSE` para mais informações.